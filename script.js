const classes = [
    ...
    { id: 'warrior', hp: 400, atk: 75, color: '#ff4444', type: 'melee', wp: 'Nihonto.png' },
    { id: 'tanker', hp: 1300, atk: 40, color: '#aaaaaa', type: 'melee', wp: 'Battle_Axe.png' },
    { id: 'mage', hp: 150, atk: 180, color: '#4444ff', type: 'ranged', pj: 'pj-magic', wp: 'Wandering_Staff.png' },
    { id: 'rogue', hp: 220, atk: 130, color: '#ff00ff', type: 'melee', wp: 'Knife.png' },
    { id: 'cleric', hp: 300, atk: 70, color: '#ffff00', type: 'ranged', pj: 'pj-magic', wp: 'Wandering_Staff.png' },
    { id: 'archer', hp: 200, atk: 115, color: '#ff8800', type: 'ranged', pj: 'pj-arrow', wp: 'Hunting_Bow.png' },
    { id: 'necro', hp: 240, atk: 140, color: '#440044', type: 'ranged', pj: 'pj-magic', wp: 'Wandering_Staff.png' },
    { id: 'paladin', hp: 600, atk: 90, color: '#ffffff', type: 'melee', wp: 'Nihonto.png' },
    { id: 'berserker', hp: 500, atk: 160, color: '#880000', type: 'melee', wp: 'Battle_Axe.png' }
];

let g = {
    active: false, shopOpen: false, time: 0, danger: 1,
    player: { x: 200, y: 200, vx: 0, vy: 0, hp: 100, maxH: 100, atk: 10, speed: 7, baseSpeed: 7, gold: 0, xp: 0, nextXp: 100, lvl: 1, dir: 1, ground: false, invul: false },
    quest: { target: 3, current: 0, type: 'kill', desc: 'DIỆT 3 QUÁI VẬT' },
...
function generateNextPlatform() {
    // KHOẢNG CÁCH NHẢY AN TOÀN
    const gapX = 100 + Math.random() * 80, gapY = (Math.random() - 0.5) * 70;
    let newY = Math.max(80, Math.min(250, g.lastY + gapY));
    ...
    if (Math.random() > 0.4) spawnEnemy(g.lastX + gapX + 200, newY + 80, g.lastX + gapX, g.lastX + gapX + newW - 40);
...
function update() {
    ...
    // Cập nhật quái vật & máu trên đầu
    g.enemies.forEach(en => {
        ...
        if (Math.abs(p.x - en.x) < 35 && Math.abs(p.y - en.y) < 40 && !p.invul) p.hp -= 1.2;
    });
...
function updateQuest() {
    ...
    if (g.quest.current >= g.quest.target) {
        g.player.gold += 300; g.player.xp += 200;
        g.quest.target += 2; g.quest.current = 0;
...
function toggleShop() {
    ...
    if (g.shopOpen) {
        document.getElementById('shop-items').innerHTML = `
            <div class="shop-card">MÁU (100G)<br><button onclick="buy('h')">MUA</button></div>
            <div class="shop-card">CÔNG+40 (400G)<br><button onclick="buy('a')">MUA</button></div>`;
        document.getElementById('full-stats').innerText = `LEVEL: ${g.player.lvl} | ATK: ${g.player.atk}`;
    }
}
...
function spawnChest(x, y) {
    const cEl = document.createElement('div'); cEl.className = 'chest'; cEl.style.left = x + 'px'; cEl.style.bottom = y + 'px';
